...
int8_t BMP280_SCK=14;   //D5
int8_t BMP280_MISO=12;  //D6
int8_t BMP280_MOSI=13;  //D7
int8_t BMP280_CS=10;    //SD3
...
/*due to unknown reasons measurement function of dht11 library doesn't work in ticker function.
so this global variable is created to store measured humidity data in loop and will be accessed in 
...
double st1147(int RawADC){
  ...
  double Temp;
  Temp=log(RawADC*10000/(1024-RawADC));
...
void data_capture() {
  ...
  /*st1147 reading*/
  if(flag[3]==1){
    if(st1147(analogRead(A0))<=40&&st1147(analogRead(A0))>0)                        //Wrong value is usually not in the range and filtered out
      data[3][data_capture_iteration] = st1147(analogRead(A0));
    else{
...
if there is a corresponding config. if no use the default config, if yes use config in database */
  ...
  if(BMP280_I2C_REQUEST_FLAG==1){
    ...
    if(flag[0]==1){                                 
      TwoWire thewire;
      thewire.pins(BMP280_SDA,BMP280_SCL);
      *(bmp280_i2c._wire)=thewire;
  ...
  /*analog reading, no setup flag because pin is pre-defined A0 and no other options*/
  if(flag[2]==1)
